...
create table public.games (
    ...
    current_round int not null default 0 check (current_round >= 0 AND current_round <= num_rounds),
...
create table public.game_round_players (
    ...
    player_id uuid not null, 
    guess_latitude float not null check (guess_latitude >= -90 AND guess_latitude <= 90),
    guess_longitude float not null check (guess_longitude >= -180 AND guess_longitude <= 180),
    score int not null default 0 check (score >= 0),
